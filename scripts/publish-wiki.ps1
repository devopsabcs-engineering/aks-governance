...
function New-DemoMarkdown {
    ...
    $descriptions = @{
        ...
        '04-aso-controllers'      = '`kubectl get pods -n azureserviceoperator-system` - the Azure Service Operator (ASO) controllers CAPZ uses to create the underlying Azure resources.'
        ...
        '08-kyverno-policies'     = '`kubectl get clusterpolicy` - the Kyverno governance policies: (A) deny container images from `docker.io` / `quay.io` (allow-list = `mcr.microsoft.com`, the customer ACR, and `registry.k8s.io`), and (B) enforce a minimum Kubernetes version on the CAPZ control-plane CR.'
